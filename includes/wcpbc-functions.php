...
function wcpbc_get_woocommerce_zipcode() {
	/* BOF ANGEPASST bez. Uncaught Error: Call to a member function get_billing_postcode() on null ***GOH */
	if (!empty(WC()->customer->get_billing_postcode())) {
		$code = WC()->customer->get_billing_postcode();
	} else {
		$code = WC()->customer->get_shipping_postcode();
	}
	/* EOF ANGEPASST bez. Uncaught Error: Call to a member function get_billing_postcode() on null ***GOH */
